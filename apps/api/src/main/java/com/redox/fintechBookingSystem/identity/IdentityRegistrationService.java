...
import com.redox.fintechBookingSystem.identity.verification.*;
...
public class IdentityRegistrationService {
  ...
  public Optional<User> registerPendingUser(String email, String password){
    String formattedEmail= validateAndNormalizeEmail(email);
    String formattedPassword=passwordPolicy.validateAndNormalize(password);
    //for security reasons, we always hash the password, even if the email already exists
    //in the database. This prevents attackers from being able to determine if an email
    // is registered based on response time or error messages.
    ...
    eventPublisher.publishEvent(
            new EmailVerificationRequested(formattedEmail,verificationToken.rawToken()));
